...
def get_api_key(provider: str) -> str:
    """
    Given str 'provider' ('anthropic', 'dashscope', 'deepseek', 'openai',
    'google', 'xai', etc), open a netrc-formatted gpg-encrypted credential
    file and search for the model provider. If successful, return the API
    secret key for the requested provider. otw None
    """
    try:
        providersL = ['anthropic', 'dashscope', 'deepseek', 'openai',
                      'google', 'x.ai']
